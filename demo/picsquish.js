...
class TaskQueue {
  ...
  #attemptAssignTask(worker) {
    const priority1Task = this.#priority1TaskQueue.shift();
    if (priority1Task)
      return this.#assignPriority1Task(worker, priority1Task);
    const priority2Task = this.#priority2TaskQueue.shift();
    if (priority2Task)
      return this.#assignPriority2Task(worker, priority2Task);
  }
  #processQueue() {
  ...
  #onTaskComplete(event) {
    ...
    switch (event.data.taskType) {
      case 0 /* CreateResizeMetadata */:
        this.#onTask1Complete(squishContext, event.data);
        break;
      case 1 /* TransformTile */:
        this.#onTask2Complete(squishContext, event.data);
        break;
    }
  ...
  add(taskData, maxWorkerPoolSize, maxWorkerPoolIdleTime) {
    workerPool.prepare((event) => this.#onTaskComplete(event), maxWorkerPoolSize, maxWorkerPoolIdleTime);
...
function squish(image, dimensionLimits, options = {}) {
  ...
  return taskQueue.add({ image, dimensionLimits, tileOptions }, maxWorkerPoolSize, maxWorkerPoolIdleTime);
}
export {
  squish
};
