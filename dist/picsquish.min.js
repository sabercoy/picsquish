var jn=Object.create;var{getPrototypeOf:Ln,defineProperty:_,getOwnPropertyNames:Gn}=Object;var wn=Object.prototype.hasOwnProperty;var En=(n,m,T)=>{T=n!=null?jn(Ln(n)):{};let k=m||!n||!n.__esModule?_(T,"default",{value:n,enumerable:!0}):T;for(let R of Gn(n))if(!wn.call(k,R))_(k,R,{get:()=>n[R],enumerable:!0});return k};var zn=(n,m)=>()=>(m||n((m={exports:{}}).exports,m),m.exports);var Bn=zn((Tk,An)=>{var v,f,c,l,s,g,Cn,qn;function Yn(n){if(n<0.5)n=0.5;var m=Math.exp(0.527076)/n,T=Math.exp(-m),k=Math.exp(-2*m),R=(1-T)*(1-T)/(1+2*m*T-k);return v=R,f=R*(m-1)*T,c=R*(m+1)*T,l=-R*k,s=2*T,g=-k,Cn=(v+f)/(1-s-g),qn=(c+l)/(1-s-g),new Float32Array([v,f,c,l,s,g,Cn,qn])}function Un(n,m,T,k,R,C){var I,M,y,D,U,q,B,A,J,$,Q,Z,K,j,G,z,L,w,Y,V,W,E,N,u,o,b,O,S,F,H;for(o=0;o<C;o++){E=o*R,N=o,u=0,I=n[E],M=I&255,y=I>>8&255,D=I>>16&255,U=I>>24&255,w=M*k[6],Y=y*k[6],V=D*k[6],W=U*k[6],j=w,G=Y,z=V,L=W,O=k[0],S=k[1],F=k[4],H=k[5];for(b=0;b<R;b++)I=n[E],q=I&255,B=I>>8&255,A=I>>16&255,J=I>>24&255,$=q*O+M*S+j*F+w*H,Q=B*O+y*S+G*F+Y*H,Z=A*O+D*S+z*F+V*H,K=J*O+U*S+L*F+W*H,w=j,Y=G,V=z,W=L,j=$,G=Q,z=Z,L=K,M=q,y=B,D=A,U=J,T[u]=j,T[u+1]=G,T[u+2]=z,T[u+3]=L,u+=4,E++;E--,u-=4,N+=C*(R-1),I=n[E],M=I&255,y=I>>8&255,D=I>>16&255,U=I>>24&255,w=M*k[7],Y=y*k[7],V=D*k[7],W=U*k[7],j=w,G=Y,z=V,L=W,q=M,B=y,A=D,J=U,O=k[2],S=k[3];for(b=R-1;b>=0;b--)$=q*O+M*S+j*F+w*H,Q=B*O+y*S+G*F+Y*H,Z=A*O+D*S+z*F+V*H,K=J*O+U*S+L*F+W*H,w=j,Y=G,V=z,W=L,j=$,G=Q,z=Z,L=K,M=q,y=B,D=A,U=J,I=n[E],q=I&255,B=I>>8&255,A=I>>16&255,J=I>>24&255,I=(T[u]+j<<0)+(T[u+1]+G<<8)+(T[u+2]+z<<16)+(T[u+3]+L<<24),m[N]=I,E--,u-=4,N-=C}}function Wn(n,m,T,k){if(!k)return;var R=new Uint32Array(n.buffer),C=new Uint32Array(R.length),I=new Float32Array(Math.max(m,T)*4),M=Yn(k);Un(R,C,I,M,m,T,k),Un(C,R,I,M,T,m,k)}An.exports=Wn});var X=4;var un=2;function t(n,m,T,k,R,C){let I=T/n,M=k/m,y=(2*C+un+1)/R;if(y>0.5)return[{toWidth:T,toHeight:k}];let D=Math.ceil(Math.log(Math.min(I,M))/Math.log(y));if(D<=1)return[{toWidth:T,toHeight:k}];let U=[];for(let q=0;q<D;q++){let B=Math.round(Math.pow(Math.pow(n,D-q-1)*Math.pow(T,q+1),1/D)),A=Math.round(Math.pow(Math.pow(m,D-q-1)*Math.pow(k,q+1),1/D));U.push({toWidth:B,toHeight:A})}return U}function e(n,m,T){let k=new Uint8ClampedArray(T.width*T.height*X);for(let R=0;R<T.height;R++){let C=((T.y+R)*m+T.x)*X,I=R*T.width*X;k.set(n.subarray(C,C+T.width*X),I)}return k}var nn=0.00001;function h(n){let m=Math.round(n);if(Math.abs(n-m)<nn)return m;return Math.floor(n)}function r(n){let m=Math.round(n);if(Math.abs(n-m)<nn)return m;return Math.ceil(n)}function kn(n,m,T,k,R,C,I,M,y,D,U){let q=k/m,B=R/T,A=h(C*q)-2*I,J=h(C*B)-2*I;if(A<1||J<1)throw new Error("Internal error in picsquish: target tile width/height is too small.");let $,Q,Z,K,j,G,z=[];for(K=0;K<R;K+=J)for(Z=0;Z<k;Z+=A){if($=Z-I,$<0)$=0;if(j=Z+A+I-$,$+j>=k)j=k-$;if(Q=K-I,Q<0)Q=0;if(G=K+J+I-Q,Q+G>=R)G=R-Q;let L={toX:$,toY:Q,toWidth:j,toHeight:G,toInnerX:Z,toInnerY:K,toInnerWidth:A,toInnerHeight:J,offsetX:$/q-h($/q),offsetY:Q/B-h(Q/B),scaleX:q,scaleY:B,x:h($/q),y:h(Q/B),width:r(j/q),height:r(G/B),initialSize:C,filterPadding:I,filter:M,unsharpAmount:y,unsharpRadius:D,unsharpThreshold:U},w=e(n,m,L);z.push({tile:w.buffer,...L})}return z}async function mn(n){let m,T,k,R,C,I;if(n.image instanceof Blob){let y=await createImageBitmap(n.image),U=new OffscreenCanvas(y.width,y.height).getContext("2d");if(!U)throw new Error("Canvas 2D context not supported");U.drawImage(y,0,0),m=U.getImageData(0,0,y.width,y.height).data,T=y.width,k=y.height,y.close();let B=n.maxDimension/T,A=n.maxDimension/k,J=Math.min(B,A,1),$=Math.floor(T*J),Q=Math.floor(k*J);I=t(T,k,$,Q,n.tileOptions.initialSize,n.tileOptions.filterPadding)}else m=n.image.from,T=n.image.fromWidth,k=n.image.fromHeight,I=n.image.stages;R=I[0].toWidth,C=I[0].toHeight;let M=kn(m,T,k,R,C,n.tileOptions.initialSize,n.tileOptions.filterPadding,n.tileOptions.filter,n.tileOptions.unsharpAmount,n.tileOptions.unsharpRadius,n.tileOptions.unsharpThreshold);return{from:m.buffer,fromWidth:T,fromHeight:k,tileTransforms:M,stages:I}}function x(n,m,T){let k=new Uint8ClampedArray(T.tile);for(let R=0;R<T.toHeight;R++){let C=R*T.toWidth*X,I=((T.toY+R)*m+T.toX)*X;n.set(k.subarray(C,C+T.toWidth*X),I)}}var p={box:{win:0.5,fn:(n)=>{if(n<0)n=-n;return n<0.5?1:0}},hamming:{win:1,fn:(n)=>{if(n<0)n=-n;if(n>=1)return 0;if(n<0.00000011920929)return 1;let m=n*Math.PI;return Math.sin(m)/m*(0.54+0.46*Math.cos(m/1))}},lanczos2:{win:2,fn:(n)=>{if(n<0)n=-n;if(n>=2)return 0;if(n<0.00000011920929)return 1;let m=n*Math.PI;return Math.sin(m)/m*Math.sin(m/2)/(m/2)}},lanczos3:{win:3,fn:(n)=>{if(n<0)n=-n;if(n>=3)return 0;if(n<0.00000011920929)return 1;let m=n*Math.PI;return Math.sin(m)/m*Math.sin(m/3)/(m/3)}},mks2013:{win:2.5,fn:(n)=>{if(n<0)n=-n;if(n>=2.5)return 0;if(n>=1.5)return-0.125*(n-2.5)*(n-2.5);if(n>=0.5)return 0.25*(4*n*n-11*n+7);return 1.0625-1.75*n*n}}};var Xn=14;function Tn(n){return Math.round(n*((1<<Xn)-1))}function d(n,m,T,k,R){let C=p[n].fn,I=1/k,M=Math.min(1,k),y=p[n].win/M,D,U,q,B,A,J,$,Q,Z,K,j,G,z,L,w,Y,V,W=Math.floor((y+1)*2),E=new Int16Array((W+2)*T),N=0,u=!E.subarray||!E.set;for(D=0;D<T;D++){U=(D+0.5)*I+R,q=Math.max(0,Math.floor(U-y)),B=Math.min(m-1,Math.ceil(U+y)),A=B-q+1,J=new Float32Array(A),$=new Int16Array(A),Q=0;for(Z=q,K=0;Z<=B;Z++,K++)j=C((Z+0.5-U)*M),Q+=j,J[K]=j;G=0;for(K=0;K<J.length;K++)z=J[K]/Q,G+=z,$[K]=Tn(z);$[T>>1]+=Tn(1-G),L=0;while(L<$.length&&$[L]===0)L++;if(L<$.length){w=$.length-1;while(w>0&&$[w]===0)w--;if(Y=q+L,V=w-L+1,E[N++]=Y,E[N++]=V,!u)E.set($.subarray(L,w+1),N),N+=V;else for(K=L;K<=w;K++)E[N++]=$[K]}else E[N++]=0,E[N++]=0}return E}function P(n){return n<0?0:n>255?255:n}function a(n){return n>=0?n:0}function In(n,m,T,k,R,C){let I,M,y,D,U,q,B,A,J,$,Q,Z=0,K=0;for(J=0;J<k;J++){U=0;for($=0;$<R;$++){q=C[U++],B=C[U++],A=Z+q*4|0,I=M=y=D=0;for(;B>0;B--)Q=C[U++],D=D+Q*n[A+3]|0,y=y+Q*n[A+2]|0,M=M+Q*n[A+1]|0,I=I+Q*n[A]|0,A=A+4|0;m[K+3]=a(D>>7),m[K+2]=a(y>>7),m[K+1]=a(M>>7),m[K]=a(I>>7),K=K+k*4|0}K=(J+1)*4|0,Z=(J+1)*T*4|0}}function Rn(n,m,T,k,R,C){let I,M,y,D,U,q,B,A,J,$,Q,Z=0,K=0;for(J=0;J<k;J++){U=0;for($=0;$<R;$++){q=C[U++],B=C[U++],A=Z+q*4|0,I=M=y=D=0;for(;B>0;B--)Q=C[U++],D=D+Q*n[A+3]|0,y=y+Q*n[A+2]|0,M=M+Q*n[A+1]|0,I=I+Q*n[A]|0,A=A+4|0;I>>=7,M>>=7,y>>=7,D>>=7,m[K+3]=P(D+8192>>14),m[K+2]=P(y+8192>>14),m[K+1]=P(M+8192>>14),m[K]=P(I+8192>>14),K=K+k*4|0}K=(J+1)*4|0,Z=(J+1)*T*4|0}}function Mn(n,m,T,k,R,C){let I,M,y,D,U,q,B,A,J,$,Q,Z,K=0,j=0;for($=0;$<k;$++){q=0;for(Q=0;Q<R;Q++){B=C[q++],A=C[q++],J=K+B*4|0,I=M=y=D=0;for(;A>0;A--)Z=C[q++],U=n[J+3],D=D+Z*U|0,y=y+Z*n[J+2]*U|0,M=M+Z*n[J+1]*U|0,I=I+Z*n[J]*U|0,J=J+4|0;y=y/255|0,M=M/255|0,I=I/255|0,m[j+3]=a(D>>7),m[j+2]=a(y>>7),m[j+1]=a(M>>7),m[j]=a(I>>7),j=j+k*4|0}j=($+1)*4|0,K=($+1)*T*4|0}}function yn(n,m,T,k,R,C){let I,M,y,D,U,q,B,A,J,$,Q,Z=0,K=0;for(J=0;J<k;J++){U=0;for($=0;$<R;$++){q=C[U++],B=C[U++],A=Z+q*4|0,I=M=y=D=0;for(;B>0;B--)Q=C[U++],D=D+Q*n[A+3]|0,y=y+Q*n[A+2]|0,M=M+Q*n[A+1]|0,I=I+Q*n[A]|0,A=A+4|0;if(I>>=7,M>>=7,y>>=7,D>>=7,D=P(D+8192>>14),D>0)I=I*255/D|0,M=M*255/D|0,y=y*255/D|0;m[K+3]=D,m[K+2]=P(y+8192>>14),m[K+1]=P(M+8192>>14),m[K]=P(I+8192>>14),K=K+k*4|0}K=(J+1)*4|0,Z=(J+1)*T*4|0}}function Vn(n,m,T){let k=3,R=m*T*4|0;while(k<R){if(n[k]!==255)return!0;k=k+4|0}return!1}function Nn(n,m,T){let k=3,R=m*T*4|0;while(k<R)n[k]=255,k=k+4|0}function Dn(n,m,T,k,R,C,I,M,y,D){let U=d(m,T,R,I,y),q=d(m,k,C,M,D),B=new Uint8Array(R*C*4),A=new Uint16Array(R*k*4);if(Vn(n,T,k))Mn(n,A,T,k,R,U),yn(A,B,k,R,C,q);else In(n,A,T,k,R,U),Rn(A,B,k,R,C,q),Nn(B,R,C);return B}var Kn=En(Bn(),1);function On(n,m,T){let k=m*T,R=new Uint16Array(k),C,I,M,y;for(let D=0;D<k;D++)C=n[4*D],I=n[4*D+1],M=n[4*D+2],y=C>=I&&C>=M?C:I>=M&&I>=C?I:M,R[D]=y<<8;return R}function Qn(n,m,T,k,R,C){let I,M,y,D,U;if(k===0||R<0.5)return;if(R>2)R=2;let q=On(n,m,T),B=new Uint16Array(q);Kn.default(B,m,T,R);let A=k/100*4096+0.5|0,J=C<<8,$=m*T;for(let Q=0;Q<$;Q++)if(I=q[Q],D=I-B[Q],Math.abs(D)>=J)M=I+(A*D+2048>>12),M=M>65280?65280:M,M=M<0?0:M,I=I!==0?I:1,y=(M<<12)/I|0,U=Q*4,n[U]=n[U]*y+2048>>12,n[U+1]=n[U+1]*y+2048>>12,n[U+2]=n[U+2]*y+2048>>12}function Jn(n){let m=Dn(new Uint8ClampedArray(n.tile),n.filter,n.width,n.height,n.toWidth,n.toHeight,n.scaleX,n.scaleY,n.offsetX,n.offsetY);if(n.unsharpAmount)Qn(m,n.toWidth,n.toHeight,n.unsharpAmount,n.unsharpRadius,n.unsharpThreshold);return m}var Sn=`
